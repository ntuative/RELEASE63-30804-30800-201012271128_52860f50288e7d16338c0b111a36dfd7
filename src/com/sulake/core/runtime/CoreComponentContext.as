package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1237:String = "asset-libraries";
      
      private static const const_1243:String = "library";
      
      private static const const_1239:String = "service-libraries";
      
      private static const const_1241:String = "library";
      
      private static const const_1244:String = "component-libraries";
      
      private static const const_1238:String = "library";
      
      private static const const_1240:String = "error_data";
      
      private static const const_1242:String = "error_cat";
      
      private static const const_1245:String = "error_desc";
       
      
      private var var_347:LibraryLoaderQueue;
      
      private var var_752:IEventDispatcher;
      
      private var var_497:uint;
      
      private var var_499:Function;
      
      private var var_251:Profiler;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            case Core.const_1037:
               debug("Core; using simple frame update handler");
               this.var_499 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_646:
               debug("Core; using complex frame update handler");
               this.var_499 = this.complexFrameUpdateHandler;
               break;
            case Core.const_1423:
               debug("Core; using profiler frame update handler");
               this.var_499 = this.profilerFrameUpdateHandler;
               this.var_251 = new Profiler(this);
               attachComponent(this.var_251,[new IIDProfiler()]);
               var_417.addChild(new ProfilerViewer(this.var_251));
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_499 = this.debugFrameUpdateHandler;
         }
      }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1240,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1242,String(param3));
         ErrorReportStorage.setParameter(const_1245,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_752 = loadingEventDelegate;
         if(this.var_347 == null)
         {
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_289,e);
         }
         try
         {
            node = config.child(const_1239)[0];
            if(node != null)
            {
               list = node.child(const_1241);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_289,e);
         }
         try
         {
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_289,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1357);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_752)
         {
            this.var_752.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_752)
         {
            ...
            this.var_752.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_752)
                  {
                     this.var_752.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_497 - this.var_347.length,this.var_497,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_499(_loc2_,_loc2_ - this.var_1830);
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(criticality < CoreComponentContext.const_141)
         {
            ...
            {
               if(this.var_617[criticality] < criticality)
               {
                  ++this.var_617[criticality];
                  skip = true;
               }
            }
            if(!skip)
         ...
         while(criticality < CoreComponentContext.const_141)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     this.var_251.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
                     error("Error in update receiver \"" + getQualifiedClassName(receiver) + "\": " + e.message,true,e.errorID,e);
      ...
      public function setProfilerMode(param1:Boolean) : void
      {
         ...
         else
         {
            if(this.var_251)
            {
               detachComponent(this.var_251);
               this.var_251.dispose();
               this.var_251 = null;
            }
            switch(this.var_1434 & 0)
            {
               case Core.const_1037:
                  this.var_499 = this.simpleFrameUpdateHandler;
                  break;
