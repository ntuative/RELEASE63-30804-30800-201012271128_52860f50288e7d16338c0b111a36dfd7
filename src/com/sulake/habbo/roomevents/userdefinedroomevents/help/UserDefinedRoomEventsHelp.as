package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private var var_1106:IContainerButtonWindow;
      
      private var var_1105:IContainerButtonWindow;
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_1738 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1105 = this.var_1738.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1105)
         {
            this.var_1105.procedure = this.onButtonClick;
         }
         this.var_1106 = this.var_1738.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1106)
         {
            this.var_1106.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_242 >= this._localizations.length - 1)
         {
            this.var_1106.disable();
         }
         else
         {
            this.var_1106.enable();
         }
         if(this.var_242 <= 0)
         {
            this.var_1105.disable();
         }
         else
         {
            this.var_1105.enable();
         }
      }
      ...
      {
         ++this.var_242;
         this.var_242 = Math.min(this.var_242,this._localizations.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_242;
         this.var_242 = Math.max(this.var_242,0);
         this.openPage();
      }
      ...
      private function localize() : void
      {
         ...
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this._localizations[this.var_242][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this._localizations[this.var_242][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this._localizations[this.var_242][1]).value;
