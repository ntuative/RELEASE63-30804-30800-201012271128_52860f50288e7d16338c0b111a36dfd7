package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : void
      {
         ...
         this.var_901 = this.getPetTypeIndexFromProduct(_loc5_.localizationId);
         this.var_338 = [];
         this.var_602 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc9_ = _loc8_.avatarRenderManager.petDataManager.getPetData(this.var_901,this);
               if(_loc9_)
               {
         ...
         while(_loc6_ < this.var_602.length)
         {
            ...
            _loc3_.push(_loc4_.getKey(this.getRaceLocalizationKey(this.var_901,_loc10_.id),this.getRaceLocalizationKey(this.var_901,_loc10_.id)));
            _loc6_++;
         }
         this._offers.add(this.var_901,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_1007,this.onApproveNameResult);
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_486 = param1.index;
         if(this.var_486 < 0 || this.var_486 > this.var_338.length)
         {
            this.var_486 = 0;
         }
         this.updateImage();
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_486 >= 0 && this.var_486 < this.var_338.length)
         {
            _loc7_ = this.var_338[this.var_486] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = this.var_602[this.var_1776] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_901,_loc3_.id,_loc2_,AvatarScaleType.const_47,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_276,AvatarAction.const_677);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_486 >= this.var_338.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_338[this.var_486] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = this.var_602[this.var_1776] as IBreed;
