package com.sulake.habbo.ui
{
   ...
   public class RoomQueueWidgetHandler implements IRoomWidgetHandler
   {
      ...
      public function processWidgetMessage(param1:RoomWidgetMessage) : RoomWidgetUpdateEvent
      {
         ...
         switch(param1.type)
         {
            case RoomWidgetRoomQueueMessage.const_678:
               this._container.roomSession.quit();
               break;
            case RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE:
               this._container.roomSession.changeQueue(RoomSessionQueueEvent.const_974);
               break;
            case RoomWidgetRoomQueueMessage.const_654:
               this._container.roomSession.changeQueue(RoomSessionQueueEvent.const_1108);
               break;
            case RoomWidgetRoomQueueMessage.const_787:
               if(this._container.catalog != null)
               {
                  this._container.catalog.openCatalogPage(CatalogPageName.const_163,true);
               }
         }
         return null;
      }
      ...
      {
         return [RoomSessionQueueEvent.const_510];
      }
      ...
      public function processEvent(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            case RoomSessionQueueEvent.const_510:
               ...
               switch(_loc2_.queueSetTarget)
               {
                  case RoomSessionQueueEvent.const_1108:
                     _loc3_ = "null";
                     break;
                  case RoomSessionQueueEvent.const_974:
                     _loc3_ = "null";
               }
               if(_loc3_ == null)
               {
               ...
               if(_loc7_.length > 1)
               {
                  if(_loc6_ && _loc2_.queueTypes.indexOf(RoomSessionQueueEvent.const_1083) != -1)
                  {
                     _loc8_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_1083) + 1;
                     _loc9_ = true;
                  }
                  else
                  {
                     _loc8_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_1300) + 1;
                  }
               }
               else
               {
