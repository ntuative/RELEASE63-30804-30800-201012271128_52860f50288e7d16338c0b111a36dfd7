package com.sulake.habbo.window
{
   ...
   public class HabboWindowManagerComponent extends Component implements IHabboWindowManager, ICoreWindowManager, IWindowFactory, IUpdateReceiver, IWindowContextStateListener
   {
      ...
      private static const const_394:uint = 3;
      
      private static const const_1231:uint = 1;
      ...
      private var var_913:IWindowRenderer;
      
      private var _skinContainer:ISkinContainer;
      
      private var var_342:Array;
      
      private var var_1405:IEventDispatcher;
      
      private var var_1138:Boolean = false;
      
      private var var_1819:ProfilerOutput;
      
      private var var_1406:ProfilerAgentTask;
      
      private var var_1404:ProfilerAgentTask;
      ...
      private function initialize(param1:IID, param2:IUnknown) : void
      {
         ...
         this.var_913 = new WindowRenderer(this._skinContainer);
         this.var_183 = new Array(const_394);
         ...
         var _loc6_:int = 0;
         while(_loc6_ < const_394)
         {
            this.var_183[_loc6_] = new WindowContext("layer_" + _loc6_,this.var_913,this,this._localization,context.displayObjectContainer,_loc5_,this);
            _loc6_++;
         }
         this.var_237 = this.var_183[const_1231];
         _loc7_ = new Bitmap(assets.getAssetByName("habbo_cursor_link_png").content as BitmapData);
         this.var_237.getMouseCursor().defineCustomCursorType(MouseCursorType.const_378,_loc7_);
         _loc7_ = new Bitmap(assets.getAssetByName("habbo_cursor_drag_png").content as BitmapData);
         this.var_237.getMouseCursor().defineCustomCursorType(MouseCursorType.const_1326,_loc7_);
         unlock();
      ...
      private function receiveProfilerInterface(param1:IID, param2:IUnknown) : void
      {
         ...
         if(_loc3_ != null)
         {
            if(!this.var_1819)
            {
               this.var_1819 = new ProfilerOutput(context,this,this.var_913);
            }
            this.var_1819.profiler = _loc3_;
            this.var_1406 = new ProfilerAgentTask("Update","Event processing");
            _loc3_.getProfilerAgentForReceiver(this).addSubTask(this.var_1406);
            this.var_1404 = new ProfilerAgentTask("Redraw","Window rasterizer");
            _loc3_.getProfilerAgentForReceiver(this).addSubTask(this.var_1404);
            this.var_1138 = true;
         }
      }
      ...
      {
         return this.var_183[param1].getDesktopWindow().getChildAt(this.var_237.getDesktopWindow().numChildren - 1);
      }
      ...
      public function update(param1:uint) : void
      {
         ...
         {
            this.var_1405.dispatchEvent(new Event(HabboWindowTrackingEvent.HABBO_WINDOW_TRACKING_EVENT_INPUT));
            _loc2_ = const_394 - 1;
            while(_loc2_ >= 0)
            {
         ...
         this.var_1405.dispatchEvent(new Event(HabboWindowTrackingEvent.HABBO_WINDOW_TRACKING_EVENT_RENDER));
         _loc2_ = 0;
         while(_loc2_ < const_394)
         {
         ...
         this.var_1405.dispatchEvent(new Event(HabboWindowTrackingEvent.HABBO_WINDOW_TRACKING_EVENT_SLEEP));
      }
      ...
      public function mouseEventReceived(param1:String, param2:IWindow) : void
      {
         if(param2 != null)
         {
            ...
            {
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1427,new Date().getTime().toString());
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1369,param2.name + ": " + param2.toString());
            }
            else if(param1 == MouseEvent.MOUSE_UP)
            {
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1462,new Date().getTime().toString());
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1406,param2.name + ": " + param2.toString());
            }
         }
      }
   }
